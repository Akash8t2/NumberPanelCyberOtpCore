...
AJAX_URL = "http://51.89.99.105/NumberPanel/client/res/data_smscdr.php"
REFERER_URL = "http://51.89.99.105/NumberPanel/client/smscdr.php"

# Bot Configuration
BOT_TOKEN = os.getenv("BOT_TOKEN") or "YOUR_BOT_TOKEN"
CHAT_IDS = ["-1003559187782", "-1003316982194"]

# Session Key (from URL parameter)
SESSKEY = os.getenv("SESSKEY") or "Q05RR0FPUUpCVg=="

# Cookies - if needed
PHPSESSID = os.getenv("PHPSESSID") or ""
...
def mask_number(number: str) -> str:
    ...
    # For Zimbabwe numbers like 263783197613
    # Format: 263***7613 (show first 3 and last 4 digits)
    if len(cleaned) >= 7:
...
def build_payload():
    """Build AJAX payload for NumberPanel"""
    ...
    params = {
        ...
        "fgcli": "",
        "fg": 0,
        "sesskey": SESSKEY,  # Important: sesskey parameter
        "sEcho": 1,
        "iColumns": 7,
        "sColumns": ",,,,,,",
        "iDisplayStart": 0,
...
def format_message(row):
    ...
    try:
        ...
        if route and isinstance(route, str):
            ...
            country_parts = re.split(r'[-]', route, 1)
            if country_parts:
...
def fetch_latest_sms():
    """Fetch latest SMS from NumberPanel website"""
    global STATE
    ...
    try:
        ...
        for idx, row in enumerate(rows):
            ...
            # Skip summary rows (they start with "0,0,0,")
...
def check_session():
    """Check if sesskey is valid"""
    try:
        params = build_payload()
        params["iDisplayLength"] = 5  # Small test
...
def print_config():
    ...
    logging.info("=" * 60)
    logging.info("🚀 NUMBERPANEL OTP BOT STARTED")
    logging.info("=" * 60)
    ...
    logging.info(f"Sesskey: {'Set' if SESSKEY else 'Not Set'}")
    logging.info(f"PHPSESSID: {'Set' if PHPSESSID else 'Not Set'}")
    logging.info("=" * 60)
...
def main():
    ...
    if check_session():
        ...
        logging.warning("⚠️ Session may be invalid. Check SESSKEY and PHPSESSID.")
